...
def main():
    ...
    repo=StorageRepository(args.db);repo.migrate();anchor=datetime.now(timezone.utc).replace(second=0,microsecond=0)
    ...
    report={"generated_server_transactions":args.events+420,"imported_records_including_client_spans":result.inserted,"services":args.services,
        ...
        "peak_process_rss_mb":round(resource.getrusage(resource.RUSAGE_SELF).ru_maxrss/1024,1),"clickhouse_bytes_on_disk":int(repo.connect().execute("SELECT sum(bytes_on_disk) FROM system.tables WHERE database=currentDatabase()").fetchone()[0]),"worker":jobs,
