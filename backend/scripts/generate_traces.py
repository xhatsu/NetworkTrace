...
SERVICES = [
    {
        ...
        "operations": [
            {"url": "/SALE_SERVICE/bpm/sale/InterfaceForSale?wsdl", "path": "/SALE_SERVICE/bpm/sale/InterfaceForSale", "route": "/SALE_SERVICE/*", "name": "SaleApi/InterfaceForSale", "method": "POST", "base_latency_us": 180000, "std_us": 30000},
            {"url": "/SALE_SERVICE/bpm/sale/InterfaceForMyViettel?wsdl", "path": "/SALE_SERVICE/bpm/sale/InterfaceForMyViettel", "route": "/SALE_SERVICE/*", "name": "SaleApi/InterfaceForMyViettel", "method": "POST", "base_latency_us": 150000, "std_us": 25000},
    ...
    },
    {
        ...
        "operations": [
            {"url": "/SALE_SERVICE/bpm/sale/InterfaceForViettelApp?wsdl", "path": "/SALE_SERVICE/bpm/sale/InterfaceForViettelApp", "route": "/SALE_SERVICE/*", "name": "CustomerService/InterfaceForViettelApp", "method": "POST", "base_latency_us": 120000, "std_us": 20000},
    ...
    },
    {
        ...
        "operations": [
            {"url": "/SALE_SERVICE/bpm/sale/InterfaceForSale?wsdl", "path": "/SALE_SERVICE/bpm/sale/InterfaceForSale", "route": "/SALE_SERVICE/*", "name": "OrderService/InterfaceForSale", "method": "POST", "base_latency_us": 220000, "std_us": 40000},
    ...
    },
    {
        ...
        "operations": [
            {"url": "/SALE_SERVICE/bpm/sale/InterfaceForChatBot?wsdl", "path": "/SALE_SERVICE/bpm/sale/InterfaceForChatBot", "route": "/SALE_SERVICE/*", "name": "NotificationService/InterfaceForChatBot", "method": "POST", "base_latency_us": 110000, "std_us": 18000},
...
# Authentic Identities directly derived from production PCAP captures with empirical traffic weights
PRINCIPALS = [
    ("cm2.0", "Basic Y20yLjA6Y20yX3ZpZXR0ZWxfYXBpX3Rva2Vu", 0.422),
    ("sale", "Basic c2FsZTp2dF9zYWxlX3Bhc3N3b3JkXzk5", 0.094),
    ("myViettel", "Basic bXlWaWV0dGVsOm15VmllVHRlbF9hcHBfdG9rZW5fOTk=", 0.092),
    ("vtp", "Basic dnRwOnZ0cF9sb2dpc3RpY3Nfa2V5", 0.091),
    ("pm_mini_app", "Basic cG1fbWluaV9hcHA6bWluaV9hcHBfc2VjcmV0XzEyMw==", 0.076),
    ("chatbot", "Basic Y2hhdGJvdDpjaGF0Ym90X2NvbnZlcnNhdGlvbl9rZXk=", 0.044),
    ("cc2.0", "Basic Y2MyLjA6Y2MyX3ZpZXR0ZWxfc2VjcmV0", 0.035),
    ("pm_vt_app", "Basic cG1fdnRfYXBwOnZ0X2FwcF9zZWNyZXQ=", 0.031),
    ("cyber_space", "Basic Y3liZXJfc3BhY2U6Y3liZXJfc2VjcmV0", 0.025),
    ("callbot_inb", "Basic Y2FsbGJvdF9pbmI6Y2FsbGJvdF9zZWNyZXQ=", 0.023),
    ("product", "Basic cHJvZHVjdDpwcm9kdWN0X2NhdGFsb2dfc2VjcmV0", 0.020),
    ("admin_ops_console", "Basic YWRtaW5fb3BzX2NvbnNvbGU6YWRtaW5fc2VjdXJlXzk5", 0.015),
    ("b2b_enterprise_client", "Basic YjJiX2VudGVycHJpc2VfY2xpZW50OmVudGVycHJpc2VfdG9rZW4=", 0.015),
    ("pos_retail_terminal", "Basic cG9zX3JldGFpbF90ZXJtaW5hbDpwYXNzd29yZF8xMjM=", 0.010),
    ("cron_batch_reconciler", "Basic Y3Jvbl9iYXRjaF9yZWNvbmNpbGVyOmJhdGNoX3Bhc3N3b3Jk", 0.005),
    ("support_agent_tier3", "Basic c3VwcG9ydF9hZ2VudF90aWVyMzpjcmVkZW50aWFsX3M3Nw==", 0.005),
]

# Principal canonical affinities during baseline (0.0h .. cutoff)
BASELINE_AFFINITIES = {
    "cm2.0": {
        "services": ["apex-edge-gateway", "apex-order-service"],
    ...
    "myViettel": {
        "services": ["apex-edge-gateway", "apex-customer-service"],
        "ips": ["10.240.147.249", "115.78.22.84"]
    },
    "vtp": {
        "services": ["apex-order-service", "apex-inventory-service", "apex-customer-service"],
    ...
    "chatbot": {
        "services": ["apex-customer-service", "apex-notification-service"],
...
def main():
    ...
    parser.add_argument("--count", type=int, default=80_000, help="Total transaction records to generate (default: 80,000)")
    parser.add_argument("--hours", type=float, default=2.0, help="Timespan in hours backwards from now (default: 2.0)")
    ...
    parser.add_argument("--output", type=str, default="/home/ubuntu/Viettel/Data/sample_abnormal_traces_50k.jsonl.gz", help="Output .jsonl.gz file")
    parser.add_argument("--preview", type=str, default="/home/ubuntu/Viettel/Data/sample_abnormal_preview.json", help="Preview JSON file with first 100 records")
    ...
    # Time-slice generation (1-minute slices = 60s)
    slice_sec = 60
    num_slices = max(1, total_window_sec // slice_sec)
    base_records_per_slice = max(1.0, args.count / float(num_slices))

    with gzip.open(args.output, "wt", encoding="utf-8", compresslevel=1) as gz_out:
        lines_buffer = []

        for slice_idx in range(num_slices):
            slice_start_sec = start_time_sec + slice_idx * slice_sec
            slice_end_sec = min(end_time_sec, slice_start_sec + slice_sec)
            slice_mid_sec = (slice_start_sec + slice_end_sec) // 2
            progress = slice_idx / float(num_slices)
            elapsed_hours = (slice_mid_sec - start_time_sec) / 3600.0

            gm = time.gmtime(slice_mid_sec)
            ctx = AnomalyContext(
                progress=progress,
                trace_time_sec=slice_mid_sec,
            ...
            # Diurnal factor: slight natural variation across day/night if >= 12h, else flat 1.0
            diurnal = 1.0
            if args.hours >= 12.0:
                diurnal = 0.85 + 0.25 * math.sin((gm.tm_hour - 6) / 24.0 * 2 * math.pi)

            # Generate records per principal in this slice
            for p_tuple in PRINCIPALS:
                p_name, auth_header, base_weight = p_tuple[0], p_tuple[1], p_tuple[2]

                # Check if principal is suppressed during baseline (new identities)
                if mgr.is_principal_suppressed_for_baseline(p_name, ctx):
                    continue

                # Check principal traffic multiplier (e.g. rate surge)
                p_mult = mgr.get_principal_traffic_multiplier(p_name, ctx)

                # Expected count for this principal in this 60s slice
                expected = base_records_per_slice * base_weight * diurnal * p_mult
                p_tx_count = int(expected) + (1 if random.random() < (expected - int(expected)) else 0)

                for _ in range(p_tx_count):
                    tx_time_sec = random.randint(slice_start_sec, slice_end_sec - 1)
                    ts_us = tx_time_sec * 1_000_000 + random.randint(1000, 999000)

                    # Choose service with affinity or weight
                    if elapsed_hours < bootstrap_cutoff_hour and p_name in BASELINE_AFFINITIES:
                        aff = BASELINE_AFFINITIES[p_name]
                        if "services" in aff and random.random() < 0.85:
                            s_name = random.choice(aff["services"])
                            svc = SVC_BY_NAME.get(s_name, SERVICES[0])
                        else:
                    ...
                    if elapsed_hours < bootstrap_cutoff_hour:
                        affinity = BASELINE_AFFINITIES.get(p_name)
                        if affinity:
                            if "ips" in affinity:
                    ...
                    t_gm = time.gmtime(tx_time_sec)
                    iso_time = f"{t_gm.tm_year:04d}-{t_gm.tm_mon:02d}-{t_gm.tm_mday:02d}T{t_gm.tm_hour:02d}:{t_gm.tm_min:02d}:{t_gm.tm_sec:02d}.{random.randint(100, 999):03d}Z"
                    ingested_iso_time = f"{t_gm.tm_year:04d}-{t_gm.tm_mon:02d}-{t_gm.tm_mday:02d}T{t_gm.tm_hour:02d}:{t_gm.tm_min:02d}:{min(59, t_gm.tm_sec+15):02d}.{random.randint(100, 999):03d}Z"
                    index_name = f"apm-apex-cluster:apm-8.11.0-transaction-{t_gm.tm_year:04d}.{t_gm.tm_mon:02d}.{t_gm.tm_mday:02d}-1"
                    ...
                    if len(lines_buffer) >= args.chunk_size:
                        ...
                        if generated % 20_000 == 0:
                            ...
                            print(f" Progress: {generated:>9,} records ({pct:5.1f}%) | {rate:,.0f} rec/s | Elapsed: {time.time() - t0:.1f}s")

            # Check service-level traffic spike (TrafficSpikeScenario)
            for s_info in SERVICES:
                s_mult = mgr.get_service_traffic_multiplier(s_info["name"], ctx)
                if s_mult > 1.0:
                    extra_spike = int(round(base_records_per_slice * 0.15 * (s_mult - 1.0)))
                    for _ in range(extra_spike):
                        tx_time_sec = random.randint(slice_start_sec, slice_end_sec - 1)
                        ts_us = tx_time_sec * 1_000_000 + random.randint(1000, 999000)
                        op = random.choice(s_info["operations"])
                        p_tuple = random.choice(PRINCIPALS)
                        p_name, auth_header = p_tuple[0], p_tuple[1]
                        client_ip = random.choice(CLIENT_IPS)
                        callers = CALLER_DEPENDENCIES.get(s_info["name"], [None])
                        caller_service = random.choice(callers)
                        node_name = random.choice(s_info["nodes"])
                        user_agent = random.choice(USER_AGENTS)
