...
|---|---|---:|---|---|---|
| `tracescope-clickhouse` | StatefulSet | 1 | `clickhouse/clickhouse-server:24.3-alpine` | High-performance analytical database | PersistentVolumeClaim (`clickhouse-data`) |
| `tracescope-storage` | StatefulSet | 1 | `xhatsu101/tracescope:app-0.2.0` | `migrate` init container, `api` container (FastAPI, built-in React UI, Agent Stats), and `analytics-worker` sidecar | None (stateless app tier connected to ClickHouse) |
| `tracescope-ingest` | Deployment | 3–12 (HPA) | `xhatsu101/tracescope:ingest-0.2.0` | Decompression, parsing, credential sanitization, normalization, and OTLP ingestion | Stateless edge pool |
...
1. **`xhatsu101/tracescope:app-<version>`**
   ...
   - Used by: `tracescope-storage` pod (`migrate` init, `api`, `analytics-worker`).
...
- `25-clickhouse-statefulset.yaml`: ClickHouse server StatefulSet.
- `26-clickhouse-service.yaml`: Internal headless/ClusterIP service for ClickHouse (`8123`, `9000`).
- `30-storage-statefulset.yaml`: App StatefulSet (`migrate` init, `api`, `analytics-worker`).
...
- `37-storage-service.yaml`: Internal headless service for storage pods.
