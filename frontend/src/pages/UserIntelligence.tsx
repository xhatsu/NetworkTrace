...
const chartColors = ["#22d3ee", "#8b5cf6", "#f59e0b", "#fb7185", "#34d399", "#60a5fa"];
...
function DonutPanel({ title, subtitle, data }: { title: string; subtitle: string; data: Array<{ name: string; value: number }> }) {
  return <Panel title={title} subtitle={subtitle}>
    <div className="grid min-h-[230px] grid-cols-[150px_1fr] items-center gap-3 p-4">
      <ResponsiveContainer width="100%" height={150}><PieChart><Pie data={data} dataKey="value" nameKey="name" innerRadius={48} outerRadius={68} paddingAngle={3} stroke="none">{data.map((_, i) => <Cell key={i} fill={chartColors[i % chartColors.length]} />)}</Pie><Tooltip {...chartTooltip} /></PieChart></ResponsiveContainer>
      <div className="space-y-2">{data.slice(0, 6).map((item, i) => <div className="flex items-center justify-between gap-3 text-xs" key={item.name}><span className="flex min-w-0 items-center gap-2 text-[#c4bdd9]"><i className="h-2 w-2 shrink-0 rounded-full" style={{ backgroundColor: chartColors[i % chartColors.length] }} /> <span className="truncate">{item.name}</span></span><b className="font-mono text-[#f5f3fa]">{n(item.value, 0)}</b></div>)}</div>
    </div>
  </Panel>;
}

function UserLinePanel({ title, subtitle, data, dataKey, color = "#22d3ee" }: { title: string; subtitle: string; data: Array<Record<string, any>>; dataKey: string; color?: string }) {
  return <Panel title={title} subtitle={subtitle}><div className="h-[230px] p-4"><ResponsiveContainer width="100%" height="100%"><LineChart data={data}><CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.06)" /><XAxis dataKey="label" stroke="#9e96b8" tick={{ fontSize: 10 }} /><YAxis stroke="#9e96b8" tick={{ fontSize: 10 }} /><Tooltip {...chartTooltip} /><Line type="monotone" dataKey={dataKey} stroke={color} strokeWidth={2.5} dot={false} activeDot={{ r: 4, fill: color }} /></LineChart></ResponsiveContainer></div></Panel>;
}
...
export function UserDetailPage() {
  ...
  const p=q.data;
  const dailyActivity = (p.daily_stats || []).map((d, i) => ({ label: new Date(d.day_start).toLocaleDateString(undefined, { month: "short", day: "numeric" }), requests: d.observation_count, index: i }));
  const relationshipMix = [
    { name: "Callers", value: p.unique_callers },
    { name: "Source IPs", value: p.unique_sources },
    { name: "Targets", value: p.unique_targets },
    { name: "Operations", value: p.unique_operations },
  ].filter(x => x.value > 0);
  return <Page eyebrow="Principal Investigation" title={principal} description="Credential origin, expected behavior, current activity, and explainable changes." actions={<div className="flex flex-wrap gap-2"><button className="btn" onClick={()=>nav(-1)}><ArrowLeft size={13}/>Back</button><button className="btn" onClick={()=>nav(`/traces?principal=${encodeURIComponent(principal)}&${qs}`)}><ExternalLink size={13}/>View traces</button><button className="btn" onClick={()=>nav(`/changes?principal=${encodeURIComponent(principal)}`)}>Related changes</button></div>}>
...
export function UserAnalyticsPage(){
  const nav=useNavigate();
  const q=useQuery({queryKey:["user-analytics"],queryFn:()=>api<Record<string,UserItem[]>>(`/api/v1/user-analytics`)});
  if(q.isLoading)return <Page eyebrow="User Intelligence" title="User Analytics" description=""><Loading/></Page>;
  if(q.error)return <Page eyebrow="User Intelligence" title="User Analytics" description=""><ErrorState message={q.error.message}/></Page>;
  const data=q.data||{};
  const sections:[[string,string,string],any[]][]=[[["most_active","Most Active Principals","Total observed credential use"],data.most_active||[]],[["most_changed","Most Changed Principals","Explainable behavior score"],data.most_changed||[]],[["shared_credentials","Shared Credential Usage","Accounts used by multiple caller services"],data.shared_credentials||[]],[["source_diversity","Source Diversity","Accounts used from many source IPs"],data.source_diversity||[]],[["most_targets","Broadest Service Access","Most target services"],data.most_targets||[]],[["most_operations","Broadest API Usage","Most operations"],data.most_operations||[]],[["newest","Newest Principals","Most recently first observed"],data.newest||[]],[["dormant_reactivated","Recently Reactivated","Returned after dormancy"],data.dormant_reactivated||[]]];
  const active=(data.most_active||[]).slice(0,10).map((u:any,i:number)=>({label:u.principal_name.length>12?`${u.principal_name.slice(0,11)}…`:u.principal_name, value:u.total_requests||0, rank:i+1}));
  const footprint=[{name:"Shared credentials",value:(data.shared_credentials||[]).length},{name:"Source diversity",value:(data.source_diversity||[]).length},{name:"Broad target access",value:(data.most_targets||[]).length},{name:"Dormant reactivation",value:(data.dormant_reactivated||[]).length}].filter(x=>x.value>0);
  return <Page eyebrow="User Intelligence" title="Identity Analytics" description="Chart-led identity posture across activity, access breadth, credential sharing, and reactivation signals.">
    <div className="grid gap-4 lg:grid-cols-[1.35fr_.65fr]"><UserLinePanel title="Activity concentration" subtitle="Top observed identities by request volume" data={active} dataKey="value" color="#22d3ee"/><DonutPanel title="Identity signal mix" subtitle="Where the strongest identity signals are concentrated" data={footprint.length?footprint:[{name:"No signals",value:1}]} /></div>
    <div className="mt-4 grid gap-4 lg:grid-cols-2">{sections.map(([meta,rows])=><Panel title={meta[1]} subtitle={meta[2]} key={meta[0]}><div className="divide-y divide-[rgba(255,255,255,0.08)]">{rows.slice(0,10).map((u:any,i:number)=><button onClick={()=>nav(`/users/${encodeURIComponent(u.principal_name)}`)} key={`${meta[0]}-${u.principal_name}`} className="flex w-full items-center justify-between p-3 text-left text-xs transition hover:bg-white/[0.05]"><span><b className="font-mono font-medium text-cyan-300">{i+1}. {u.principal_name}</b><small className="ml-2 text-[#9e96b8]">{u.principal_type||"shared usage"}</small></span><span className="font-mono font-medium text-[#f5f3fa]">{n(u.behavior_score??u.callers??u.unique_sources??u.unique_targets??u.unique_operations??u.total_requests,0)}</span></button>)}{!rows.length&&<div className="p-4 text-xs text-[#c4bdd9]">No qualifying principals.</div>}</div></Panel>)}</div>
  </Page>;
}
